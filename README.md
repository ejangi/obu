...
1. Create a crypt remote named `obu-crypt` with `rclone config`: choose `crypt`, set its underlying remote to `onidel:ejangi-nix`, use standard filename encryption and directory-name encryption, then set a strong password and salt. Do not commit or put these values in this repository.
2. Copy `config.example.toml` to `config.toml`, replace both mounted-drive paths, and optionally set a stable hostname.
3. Review the `[filters.common]`, `[filters.primary]`, and `[filters.secondary]` sections in `config.toml`. They are the persistent blacklist and use native rclone filter syntax. Keep only files that can be safely recreated; a leading `-` excludes a pattern. Each drive selects its ordered groups with `filters = ["common", "primary"]` (or `secondary`).
4. Review the planned commands, then simulate the transfer:
...
5. Enable the nightly systemd user timer:
