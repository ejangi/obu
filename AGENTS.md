...
- `[filters.*]` in `config.toml`: persistent, reviewed backup blacklist. `common` applies to every source; source groups hold source-specific exclusions.
...
- Name configured backup sources in ordinal sequence: `primary`, `secondary`, `tertiary`, then later ordinals if needed.
...
- Keep ordinary backups copy-only and make source-removing `sync` explicit; retain post-transfer `rclone cryptcheck --one-way` verification.
